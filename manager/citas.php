...
if ($filtro == 'diarias') {
	$sql = "SELECT id, id_mascota, id_cita, title, start, end, color FROM calendario WHERE DATE(start) = CURDATE()";
} else if ($filtro == 'urgencias') {
	$sql = "SELECT c.id, c.id_mascota, c.id_cita, c.title, c.start, c.end, c.color FROM calendario c JOIN citas ci ON c.id_cita = ci.id_cita WHERE ci.motivo LIKE '%urgencia%'";
} else {
	$sql = "SELECT id, id_mascota, id_cita, title, start, end, color FROM calendario";
}
